...
def registrar_usuario():
  '''
  Esta función solicita al usuario todos los datos necesarios para registro
  '''
  ...
  while continuar:
    ...
    # Verificar que la placa no esté ingresada
    if any(BD['Placa'] == placa):
      print("La placa ya se encuentra registrada \n")
    else:
      continuar = False
  ...
  timezone = pytz.timezone('America/Bogota') # Bogotá, Colombia timezone
  ahora = dt.datetime.now(timezone).strftime("%d/%m/%Y %H:%M:%S")
  nueva_fila = pd.DataFrame({'Placa': [placa], 'Nombres': [nombre],
                              'Apellidos': [apellido], 'Documento': [documento],
                              'Fecha creación': ahora})
  return nueva_fila
...
# Creación de la base de datos para almacenar la información
BD = pd.DataFrame(columns=['Placa', 'Nombres', 'Apellidos', 'Documento', 'Fecha creación'])
...
while bandera:
  ...
  if seleccion == 1:
    BD = pd.concat([BD, registrar_usuario()], ignore_index=True)
